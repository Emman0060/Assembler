...
namespace Assembler
{
    public partial class Machine : Form
    {
        ...
        private async void Interpret()
        {
            ...
            //"PC: ", "CIR: ", "MAR: ", "MDR: ", "ACC: ", "INPUT: ", "OUTPUT: "
            while (ProgramIsRunning)
            {
                ...
                MemoryDataRegister = MemoryArray[ProgramCounter];
                ...
                if (MemoryDataRegister != 0 )
                {
                        ...
                        if (InstructionAndBitPatterns.ContainsValue(Convert.ToInt32(Instruction, 2)))
                        {
                            ...
                            if (TheInstruction == "STR")
                            {
                                if (AddressingMode == "01")
                                {
                                    ...
                                    Console.WriteLine(GeneralRegisterArray[RegisterAddress]);
                            ...
                            if (TheInstruction == "CMP")
                            {
                                ...
                                if (AddressingMode == "11")
                                {
                                    ...
                                    if (InstructionAndBitPatterns.FirstOrDefault(x => x.Value == Convert.ToInt32(BranchingInstruction.Substring(0, 7), 2)).Key == "BLT")
                                    {
                                        if (RegisterAddress < CompareWith)
                                        {
                                            ProgramCounter = Convert.ToInt32(BranchingInstruction.Substring(7, 4), 2) - 22;
                    ...
                    if (MachineCode.Length == 7)
                    {
                        ...
                        if (InstructionAndBitPatterns.FirstOrDefault(x => x.Value == Convert.ToInt32(MachineCode.Substring(0,7),2)).Key == "HALT")
                        {
        ...
        private void LoadIntoMemory()
        {
            ...
            for (int i = 0; i < LineOfInstructions.Count; i++)
            {
                ...
                if (LineOfInstructions.Contains("HALT"))
                {
                    MachinePattern = 1001010;
                    MemoryArray[i] = MachinePattern;
                }
