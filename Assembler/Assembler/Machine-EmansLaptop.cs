...
namespace Assembler
{
    public partial class Machine : Form
    {
        ...
        {
                SourceCode = UserInputBox.Text.Trim();
                return SourceCode;
        }
        ...
        private async void Run()
        {
            ...
            int RegisterAddress1 = 0;
            int RegisterAddress2 = 0;
            int MemoryLocationAddress = 0;
            int Operand = 0;
            NewUserCode = ResolveLabels();
            string[] NewInputWithoutLabels = NewUserCode.ToArray();

            if (ValidSyntax()[ValidSyntax().Count - 1] != 0)
            {
                DisplayErrors();
                MessageBox.Show("Error in syntax, check error menu.");
                return;
            }

            for (int i = 0; i < NewInputWithoutLabels.Length; i++)
            {
                while (ProgramIsRunning)
                {
                    if (BranchingLabels.ContainsKey(NewInputWithoutLabels[i]))
                    {
                        i = BranchingLabels[NewInputWithoutLabels[i]];
                    }

                    if (NewInputWithoutLabels[i] == "INP")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));

                        GeneralRegisterArray[RegisterAddress1] = Operand;
                        GeneralRegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(Operand);
                        await Task.Delay(1500);
                    }

                    if (NewInputWithoutLabels[i] == "LDRM")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        MemoryLocationAddress = Convert.ToInt32(NewInputWithoutLabels[i + 2]);

                        MemoryArray[RegisterAddress1] = MemoryLocationAddress;
                        MainMemoryArrayValue[RegisterAddress1].Text = Convert.ToString(MemoryLocationAddress);

                    }

                    if (NewInputWithoutLabels[i] == "LDR")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        MemoryLocationAddress = Convert.ToInt32(NewInputWithoutLabels[i + 2]);

                        RegisterArray[RegisterAddress1] = MemoryArray[MemoryLocationAddress];
                        RegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(MemoryArray[MemoryLocationAddress]);
                    }

                    if (NewInputWithoutLabels[i] == "STR")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        MemoryLocationAddress = Convert.ToInt32(NewInputWithoutLabels[i + 2]);

                        MemoryArray[MemoryLocationAddress] = RegisterArray[RegisterAddress1];
                        MainMemoryArrayValue[MemoryLocationAddress].Text = Convert.ToString(RegisterArray[RegisterAddress1]);
                    }

                    if (NewInputWithoutLabels[i] == "ADD")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 3]);

                        ValueHolder = Operand + GeneralRegisterArray[RegisterAddress2];
                        GeneralRegisterArray[RegisterAddress1] = ValueHolder;
                        GeneralRegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(ValueHolder);
                    }

                    if (NewInputWithoutLabels[i] == "SUB")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        MemoryLocationAddress = Convert.ToInt32(NewInputWithoutLabels[i + 3]);

                        ValueHolder = MemoryLocationAddress - GeneralRegisterArray[RegisterAddress2];
                        GeneralRegisterArray[RegisterAddress1] = ValueHolder;
                        GeneralRegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(ValueHolder);
                    }

                    if (NewInputWithoutLabels[i] == "MOV")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 2]);

                        GeneralRegisterArray[RegisterAddress1] = Operand;
                        GeneralRegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(ValueHolder);
                    }

                    if (NewInputWithoutLabels[i] == "CMP")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 2]);
                    ...
                    if (NewInputWithoutLabels[i] == "AND")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 3]);

                        ValueHolder = GeneralRegisterArray[RegisterAddress2] & Operand;
                        GeneralRegisterArray[RegisterAddress1] = ValueHolder;
                        GeneralRegistersArrayValueLabel[RegisterAddress1].Text = Convert.ToString(ValueHolder);
                    }

                    if (NewInputWithoutLabels[i] == "ORR")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 3]);
                    ...
                    if (NewInputWithoutLabels[i] == "EOR")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 3]);
                    ...
                    if (NewInputWithoutLabels[i] == "MVN")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 2]);
                    ...
                    if (NewInputWithoutLabels[i] == "LSL")
                    {
                        RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                        RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                        Operand = Convert.ToInt32(NewInputWithoutLabels[i + 3]);
                    ...
                    if (NewInputWithoutLabels[i] == "LSR")
                    {
                        {
                            RegisterAddress1 = Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1));
                            RegisterAddress2 = Convert.ToInt32(NewInputWithoutLabels[i + 2].Substring(1));
                    ...
                    if (NewInputWithoutLabels[i] == "CMP")
                    {
                        if (NewInputWithoutLabels[i + 3] == "B")
                        {
                            i = BranchingLabels[NewInputWithoutLabels[i]];
                        }
                        if (NewInputWithoutLabels[i + 3] == "BEQ")
                        {
                            if (GeneralRegisterArray[Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1))] == Convert.ToInt32(NewInputWithoutLabels[i + 2]))
                            {
                                i = BranchingLabels[NewInputWithoutLabels[i + 4]];
                            }
                            else
                            {
                                continue;
                        ...
                        if (NewInputWithoutLabels[i + 3] == "BNE")
                        {
                            if (GeneralRegisterArray[Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1))] != Convert.ToInt32(NewInputWithoutLabels[i + 2]))
                            {
                                i = BranchingLabels[NewInputWithoutLabels[i + 4]];
                        ...
                        if (NewInputWithoutLabels[i + 3] == "BGT")
                        {
                            if (GeneralRegisterArray[Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1))] > Convert.ToInt32(NewInputWithoutLabels[i + 2]))
                            {
                                i = BranchingLabels[NewInputWithoutLabels[i + 4]];
                        ...
                        if (NewInputWithoutLabels[i + 3] == "BLT")
                        {
                            if (GeneralRegisterArray[Convert.ToInt32(NewInputWithoutLabels[i + 1].Substring(1))] < Convert.ToInt32(NewInputWithoutLabels[i + 2]))
                            {
                                i = BranchingLabels[NewInputWithoutLabels[i + 4]];
                    ...
                    if (NewInputWithoutLabels[i] == "HALT")
                    {
        ...
        private string[] Tokenisation()
        {
            ...
            string NormalisedInput = UserInputBox.Text.ToUpper();
        ...
        {
            List<int> LineOfErrorsAndNumberOfErrors = new List<int>();
            LineOfErrorsAndNumberOfErrors = ValidSyntax();
        ...
        private void UserInputBox_KeyDown(object sender, KeyEventArgs e)
        {
            for (int i = 0; i <= UserInputBox.Lines.Count(); i++)
            {
                ...
                {
                    if (!LineCountTextBox.Text.Contains(i.ToString()))
                    {
                        LineCountTextBox.Text += i.ToString() + "\n";
                    }
                }
                else
                {
                    LineCountTextBox.Clear();
                }
            }
        }
                        ...
                        if (InstructionAndBitPatterns.ContainsValue(Convert.ToInt32(Instruction, 2)))
                        {
                            ...
                            if (TheInstruction == "INP")
                            {
                                if (AddressingMode == "01")
                                {
                                    ...
                                    GeneralRegisterArray[RegisterAddress] = RegisterArray[Convert.ToInt32(Operand1,2)];
                                    GeneralRegistersArrayValueLabel[RegisterAddress].Text = Convert.ToString(RegisterArray[Convert.ToInt32(Operand1, 2)]);
                            ...
                            if (TheInstruction == "CMP")
                            {
                                ...
                                if (Convert.ToString(MemoryArray[ProgramCounter + 1], 2) == "BEQ")
                                {

                                }
                                if (Convert.ToString(MemoryArray[ProgramCounter + 1], 2) == "BNE")
                                {

                                }
                                if (Convert.ToString(MemoryArray[ProgramCounter + 1], 2) == "BGT")
                                {

                                }
                                if (Convert.ToString(MemoryArray[ProgramCounter + 1], 2) == "BLT")
                                {
                ...
                for (int j = 0; j < LineItems.Count; j++)
                {
                    if (InstructionAndBitPatterns.ContainsKey(LineItems[j]))
                    {
                        ...
                        if (LineItems[j] == "B" || LineItems[j] == "BGT" || LineItems[j] == "BLT" || LineItems[j] == "BNE" || LineItems[j] == "BEQ")
                        {
                            ...
                            MachinePattern += BranchingLabels.FirstOrDefault(x => x.Key == LineItems[j+1]).Value;
                        }
                    }
                    ...
                    {
                        MachinePattern = MachinePattern << 3;
                        MachinePattern += j;
                    }
